...
class ProgramaViewTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.centro_de_formacion = Centro_de_formacion.objects.create(id=1,centro_de_formacion='CENTRO DE COMERCIO Y SERVICIOS')
        self.nivel_formacion = Nivel_formacion.objects.create(id =1,nivel_formacion='CURSO ESPECIAL')
        self.nombre_programa_f = Programa.objects.create(id =1,nombre_programa_f='Program Name')
    ...
    def test_programa_view_with_filters(self):
        # Simula una solicitud GET con parámetros de filtro
        response = self.client.get(reverse('cores:programa'), {
            'centro_de_formacion': '1',
            'nivel_formacion': '1',
            'programa_formacion': '1',
            'modalidad': '1'  
        })

        # Verifica que al menos un registro coincida con los filtros
        lista_filtros = P04.objects.filter(
            fecha_p04=timezone.now(),
            codigo_regional='REG001',
            nombre_regional='REGIONAL CAUCA',
            codigo_centro='CEN001',
        ...
        print("Lista de filtros:", lista_filtros)
        self.assertEqual(lista_filtros.count(), 1)
